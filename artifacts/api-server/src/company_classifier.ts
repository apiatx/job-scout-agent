 ...
 *
 * hasPreferredIndustrySignal() — used specifically for the Perplexity Quick Search
 * description-relevance gate.  Quick Search returns any company on ATS platforms;
 * this function checks whether the job's description snippet actually contains signals
 * that match the user's preferred industry categories, so off-target companies
 * (advertising agencies, construction firms, healthcare SaaS, etc.) are dropped
 * before Claude scoring — dynamically, from user settings, with no hardcoded names.
 */
...
export const INDUSTRY_SIGNAL_MAP: Partial<Record<IndustryCategory, IndustrySignals>> = {
...
export function getFilterDecision(
  ...
  // Drop when:
  //   a) user has preferred categories
  //   b) high OR medium confidence in the classification (name or title level)
  //   c) detected category is explicitly wrong (generic_saas or healthcare/staffing)
  //   d) category does NOT match any preferred category
  //
  // 'unknown' industry always passes — can't be sure, let Claude or desc-gate decide.
  if (preferredCategories.size > 0) {
    ...
    if (isDefinitelyWrong && !preferredCategories.has(cls.industryCategory) &&
        (cls.confidence === 'high' || cls.confidence === 'medium')) {
...
// ── Description-relevance gate (Quick Search / Perplexity only) ───────────────
//
// When Perplexity returns job postings from open ATS platforms (Greenhouse,
// Lever, etc.), it can return any company that happens to post there —
// regardless of whether they're in the user's target industries.
//
// This function checks whether the combined title+description snippet of a
// Perplexity-discovered job contains at least one signal from the user's
// preferred industry categories.
//
// It is built DYNAMICALLY from the user's preferred categories (mapped from
// their saved industries setting) — no company names are hardcoded.
//
// Usage: apply ONLY to jobs with source='Perplexity' and description.length > 100.
// If description is empty/short, pass through (no content to judge).

export function hasPreferredIndustrySignal(
  text: string,
  preferredCategories: Set<IndustryCategory>,
): boolean {
  for (const [cat, signals] of Object.entries(INDUSTRY_SIGNAL_MAP) as [IndustryCategory, IndustrySignals][]) {
    if (!preferredCategories.has(cat as IndustryCategory)) continue;
    if (!signals) continue;
    // Check all three signal layers (name, title, desc patterns)
    for (const p of [
      ...(signals.namePatterns  ?? []),
      ...(signals.titlePatterns ?? []),
      ...(signals.descPatterns  ?? []),
    ]) {
